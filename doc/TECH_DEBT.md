...
## TD-5: 贝叶斯搜索默认预算下退化为随机搜索 —— **已修复（2026-07-21）**
...
`n_startup_trials` 不再使用 Optuna 默认值，改为随预算缩放
（`_tpe_startup_trials`：约 1/3 用于探索，下限 3、上限 10）。
默认预算 10 → 3 次随机 + 7 次 TPE 建模。

这样**不增加默认耗时**（trial 数不变）就恢复了贝叶斯行为，
避开了"提高预算 vs 改名"的取舍。

实测（20 个随机种子，`n_trials=10`，目标函数最优值越小越好）：

| | 最优值中位数 |
|---|---|
| 修复前（`n_startup=10`，全随机） | 10.574 |
| 修复后（`n_startup=3`，TPE 生效） | 4.997 |

20 次中 13 次修复后更优。

`budget_config.n_startup_trials` 可显式覆盖。

### 仍待产品决策（不阻断）

默认 10 次 trial 对贝叶斯而言仍然偏少。若希望搜索质量进一步提升，
需要把 `n_trials_per_model` 默认提到 30+，代价是默认耗时约 3 倍。
