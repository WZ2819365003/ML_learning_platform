...
export function buildPredictedActualOption(actual = [], predicted = [], windowSize = 200) {
  const n = Math.min(actual.length, predicted.length)
  const size = Math.min(windowSize, n)
  const from = Math.max(0, n - size)          // most recent slice
  const idx = Array.from({ length: size }, (_, i) => from + i + 1)
...
export default function PredictedActualCurve({ payload, height = 320 }) {
  ...
  const option = useMemo(
    () => buildPredictedActualOption(actual, predicted, windowSize),
    [actual, predicted, windowSize],
  )
    ...
    <div>
      <Space size={8} style={{ marginBottom: 6 }} wrap>
        ...
        <Radio.Group size="small" value={windowSize} onChange={e => setWindowSize(e.target.value)}
          options={WINDOW_OPTIONS.filter(w => w <= Math.max(total, WINDOW_OPTIONS[0]))
            .map(w => ({ label: w, value: w }))}
          optionType="button" buttonStyle="solid" />
        <Text type="secondary" style={{ fontSize: 12 }}>条 / 共 {total} 条，可框选缩放</Text>
      </Space>
