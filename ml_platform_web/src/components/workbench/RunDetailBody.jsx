/**
 * RunDetailBody — the tabbed detail view for one ExperimentRun.
 *
 * Extracted from RunInspector so the same body can render in two shells
 * without the two drifting apart: the Drawer (quick peek from a list) and the
 * standalone model detail page (deep link, own URL, own back button).
 * Everything here is shell-agnostic — no Drawer/page chrome, no navigation.
 *
 * `variant` only tunes density: 'drawer' keeps the compact 780px-wide layout,
 * 'page' gets more breathing room and a taller log/chart area.
 */
import React, { useEffect, useRef, useState } from 'react'
import {
  Descriptions, Tag, Space, Table, Tabs, Empty, Spin,
...
export function RunDetailBody({ runId, active = true, defaultTab = 'overview', onData }) {
  const [loading, setLoading] = useState(false)
  const [data, setData] = useState(null)
  const [error, setError] = useState(null)
  const [activeTab, setActiveTab] = useState(defaultTab)
  // Kept in a ref so a caller passing an inline arrow does not retrigger the fetch.
  ...
  useEffect(() => {
    if (active) setActiveTab(defaultTab)
  }, [active, defaultTab, runId])
  ...
  useEffect(() => {
    ...
    platformRunsApi.inspect(runId, { log_limit: 200 })
      .then(resp => { setData(resp); onDataRef.current?.(resp) })
        ...
        {!loading && !error && data && (
          <Tabs
            ...
            items={[
              ...
              // ── Evaluation detail ──
              // Lazily mounted: `destroyInactiveTabPane` on the Tabs keeps the
              // five extra endpoints from firing until the tab is opened.
              {
                key: 'evaluation',
                label: '评估细节',
                children: (
