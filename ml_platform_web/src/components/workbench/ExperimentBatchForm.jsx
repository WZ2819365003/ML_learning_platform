        ...
        {gridKeys.map(param => {
          ...
          return (
            <div key={param}>
              ...
              {isNum ? (
                <InputNumber style={{ width: '100%' }} placeholder={`默认 ${first}`}
                  value={overrides[param]} onChange={(v) => onChange({ [param]: v })} />
              ) : (
                <Select style={{ width: '100%' }} placeholder={`默认 ${JSON.stringify(first)}`}
                  value={overrides[param]} allowClear
          ...
          return (
            <div key={param}>
              ...
              <Select mode="tags" style={{ width: '100%' }}
                value={current.map(v => JSON.stringify(v))} tokenSeparators={[',']}
                onChange={(vals) => {
            ...
            <div key={param} style={{ padding: '8px 12px', borderRadius: 6, background: 'rgba(139, 92, 246, 0.06)', border: '1px solid rgba(139, 92, 246, 0.15)' }}>
              ...
              {(d.type === 'float' || d.type === 'int') && (
                <Row gutter={8} style={{ marginTop: 6 }}>
                  <Col span={12}><InputNumber size="small" style={{ width: '100%' }} placeholder="low"
                    value={active.low} onChange={(v) => onChange({ [param]: { ...active, low: v } })} /></Col>
                  <Col span={12}><InputNumber size="small" style={{ width: '100%' }} placeholder="high"
                    value={active.high} onChange={(v) => onChange({ [param]: { ...active, high: v } })} /></Col>
                </Row>
              )}
            </div>
          )
...
/**
 * ExperimentBatchForm — strategy/tuning batch configuration (baseline / grid /
 * bayesian + per-model search space + budget + apply-plan). Extracted from the
 * old ExperimentBatchModal so it can live inline (config step "调参策略" tab) or
 * inside a Modal shell.
 *
 * Activation is driven by the explicit `active` prop (not a Modal `open`) so
 * reset/data-loading only fire when the surface is actually shown. `resetKey`
 * changing (with active true) forces a fresh reset.
 *
 * props: task, active (bool), resetKey, onSubmitted
 */
export default function ExperimentBatchForm({ task, active = true, resetKey, onSubmitted }) {
  ...
  useEffect(() => {
    if (!active || !task?.task_type) return
    setLoading(true)
  ...
  const applyPlan = (planId) => {
    ...
    if (plan.strategy_type === 'grid_search') setModelParams({ grid: savedSpace })
    else if (plan.strategy_type === 'bayesian_search') setModelParams({ distribution: savedSpace })
    else setModelParams({ overrides: savedSpace })
  ...
  const handleSubmit = async () => {
    ...
    if (!values.selected_models?.length) { message.warning('请至少选择一个模型'); return }
    ...
    const budget_config = { max_trials: values.max_trials, cv_folds: values.cv_folds, test_size: values.test_size }
    if (strategy === 'bayesian_search') budget_config.n_trials_per_model = values.n_trials_per_model
      ...
      <Form form={form} layout="vertical" size="middle">
        ...
        <Form.Item name="selected_models" label="参与模型" rules={[{ required: true, message: '请至少选择一个模型' }]}>
          <Select mode="multiple" loading={loading}
            ...
            options={modelOptions} maxTagCount="responsive" />
        </Form.Item>
              ...
              items={selectedModels.map(m => ({
                ...
                children: (
                  <div style={{ padding: '4px 2px' }}>
                    <ModelTuningPanel strategy={strategy} modelKey={m} spec={tuningSpaces[m]}
                      value={modelParams} onChange={(patch) => updateModelParam(m, patch)} />
                  </div>
                ),
              }))} />
          </>
